  ...
  QWidget(parent)
{
  ...
  mSendControllerIcon = new QIcon(QPixmap(iconsPath + "send.png"));
  mStopControllerIcon = new QIcon(QPixmap(iconsPath + "stop.png"));
  ...
  mSSH = NULL;
  mRemoteEnable = false;
  mRemoteControlIcon = new QIcon(QPixmap(iconsPath + "remote.png"));
  ...
  mUninstallButton->setIcon(QIcon(QPixmap(iconsPath + "uninstall.png")));
...
void TransferWidget::makeDefaultControllerWarning(int s) {
  if (s) {
    ...
    msgBox.setText("Checking this box will install the current controller as a default controller on the real DARwIn-OP robot.");
    msgBox.setInformativeText("After upload, the controller will run automatically when the robot starts.\n\n"
                              "Warning: the initial position of the robot will not be checked, so make sure that the robot is at the correct initial position before starting it.");
    msgBox.setStandardButtons(QMessageBox::Ok);
    msgBox.setDefaultButton(QMessageBox::Ok);
